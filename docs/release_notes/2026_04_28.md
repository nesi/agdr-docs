---
title: 28 April 2026
description: "AGDR release notes for 28 April 2026"
---

## Versions

- Dictionary: 2025_09_01
- Submission (sheepdog): v2026.02
- Portal: production-7.2.4-2026.02
- AGDR Internal helm chart: 1.4-0.2.13
- REMS: 1.0


### Internal micro-services and more

- gen3-helm chart: 0.2.13
- indexd: 2026.02
- fence: 2026.02
- arborist: 2026.02
- peregrine: 2026.02
- sheepdog: 2026.02
- guppy: 2026.02
- Elasticsearch: elasticsearch-oss:7.10.2
- nginx (revproxy): 2026.02
- tube: 2026.02
- spark: 2026.02
- manifestservice: 2026.02
- metadata: production-1.2.2-2026.02
- postgres: 16.4.0

Note: *JupyterHub* is excluded as not used in AGDR.


## New and Improved
  
- Correct the raw and processed file number display bug in project discovery
- Correct the metadata sync failure bug when the third-party API is not accessible
- Change user login to keycloak
- Replace tls certification from ZeroSSL to Letsencrypt
